...
*/
{
  ...
  async function SGRReset()
  // Resets const objects created with SGRCreate()
  {
    const v = new SGRCreate()
    for (const key in v) {
      //console.log(`reset: `, key, v[key])
      toggleSGR[key] = v[key]
    }
  }

  function Ecma48Create() {
  ...
  async function Ecma48Reset()
  // Resets const objects created with Ecma48Create()
  {
    const v = new Ecma48Create()
    for (const key in v) {
      ecma48[key] = v[key]
    }
  }
  ...
  // @verbose     Spam the console on information about each discovered control function.
  {
    ...
    switch (sauce.version) {
      case `00`:
        this.font = handleFontSauce(sauce)
        if (sauce.config.width > 80) {
          // TODO SAUCE TInfo binary gets corrupted by the browser, so un-cap the maxColumns
          if (sauce.config.width <= 180) cursor.maxColumns = sauce.config.width
          else {
...
Letter Spacing: ${sauce.config.letterSpacing}\nANSI Flags: ${sauce.config.flags}`)
    ...
    // note: This function may apply toggleSGR = new SGRCreate() // reset to defaults
    ...
    if (ecma48DOM.html.endsWith(`\u241B\u005B\uFFFD\uFFFD`) === true) { // hack fix for tail `␛[��`
      ...
      ecma48DOM.html = `${ecma48DOM.html.slice(0, -27)}</i></div>`
    } else if (ecma48DOM.html.endsWith(`</i></div>`) === false) {
      ecma48DOM.html = `${ecma48DOM.html}</i></div>`
    } else if (ecma48DOM.html.endsWith(`</div>`) === true) {
      ecma48DOM.html = `${ecma48DOM.html.slice(0, -6)}</i></div>`
    }
    ...
    for (let line of cursor.eraseLines) {
      ...
      ecma48DOM.html = ecma48DOM.html.replace(exp, `<div id="row-${line}" class="ED">`)
    }
  ...
  // @items  An array containing Unicode decimal text
  {
    ...
    function itag() {
      switch (item.value.split(`+`)[0]) { // get CSI name
        case `SGR`: return renditionItalic(item.value)
        default: return renditionItalic()
      }
    }
    ...
    if (lineWrap === false) {
      ...
      items = items.filter((item) => { return (item !== (undefined || null) && item.length !== 0) })
    }
  ...
  // @s       String of text
  {
    ...
    if (verbose) {
      ...
      console.log(`Time taken to process buildDecimalArray: ${(d1 - d0)} milliseconds`)
    }
    return ua
  }
  ...
  // @v   Array of text characters represented in Unicode decimal values
  {
    ...
    for (const chr of v) {
      ...
      if (hvp.mode === `n`) hvp.column = hvp.column.concat(String.fromCharCode(chr)) // keep concat to avoid \n injection
    ...
    if (verbose) console.log(`Cursor repositioned to row ${hvp.row} and column ${hvp.column}`)
  ...
  //          ANSI.SYS permits '=' '?' '>' ie ←[=13h and ←[?13h
  {
    ...
    if (v.length > 1 && v[1] > 0) sm.value = `${String.fromCharCode(v[0])}${String.fromCharCode(v[1])}`
  ...
  {
    if (v >= 40 && v <= 49 || v >= 480 && v <= 489 || v >= 4810 && v <= 4899 || v >= 48100 && v <= 48255) {
  ...
  // @v   Decimal value of a CSI
  {
    if (v >= 30 && v <= 39 || v >= 380 && v <= 389 || v >= 3810 && v <= 3899 || v >= 38100 && v <= 38255) {
  ...
  function ParseSetMode(sm = -1) {
    ...
    // set colour depth
    if ([0, 2, 5, 6, 15, 17].includes(sm)) { this.colorDepth = 1; v[0] = `1-bit` } // monochrome 1-bit colour
    else if ([5, 6].includes(sm)) { this.colorDepth = 0; v[0] = `4-bit monochrome` } // grey scale 4-bit colour
    else if ([1, 3, 4].includes(sm)) { this.colorDepth = 2; v[0] = `2-bit` } // magenta 2-bit colours
    else if ([13, 14, 16, 18].includes(sm)) { this.colorDepth = 4; v[0] = `4-bit` } // 4-bit colours
    else if (sm === 19) { this.colorDepth = 8; v[0] = `8-bit [unsupported]` } // 8-bit colours
    // set resolution (but in our HTML/CSS output we only switch the font)
    if (sm === 2) this.font = 19 // MDA font 80×25
    else if ([0, 1, 4, 5, 13, 19].includes(sm)) this.font = 12 // CGA font
    else if ([6, 14].includes(sm)) this.font = 13 //  CGA higher resolution
    else if ([3, 15, 16].includes(sm)) this.font = 15 // EGA font
  ...
  // @vals          SGR parameter values or ANSI.SYS text attributes
  {
    ...
    for (const v of values) {
      ...
      if (val === 0 && vals !== `ICE+0`) {
        if (verbose) console.info(`SGRCreate()`)
        SGRReset()
        //toggleSGR = new SGRCreate() // reset to defaults
      }
          ...
          if ([38, 48].includes(val) && values[2] === `2`) {
            ...
            if (r >= 0 && r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255) {
              ...
              if (val === 38) {
                ...
                if (toggleSGR.rgbB.length > 0) this.styles += toggleSGR.rgbF + toggleSGR.rgbB
                else this.styles += toggleSGR.rgbF
              } else if (val === 48) {
                toggleSGR.colorB = null
                toggleSGR.rgbB = `background-color:rgb(${r},${g},${b});`
                if (toggleSGR.rgbF.length > 0) this.styles = toggleSGR.rgbF + toggleSGR.rgbB
                else this.styles += toggleSGR.rgbB
              }
              continue
            }
            ...
            toggleSGR.rgbF = ``
            if (toggleSGR.rgbB.length > 0) this.styles = toggleSGR.rgbB
          } else if (findBackground(val) === true) {
            toggleSGR.colorB = val
            toggleSGR.rgbB = ``
            if (toggleSGR.rgbF.length > 0) this.styles = toggleSGR.rgbF
          }
      }
    ...
    if (toggleSGR.bold === true && toggleSGR.colorF !== 38 && toggleSGR.colorF >= 30 && toggleSGR.colorF <= 39) this.classes += ` SGR1${toggleSGR.colorF}`
    else if (toggleSGR.colorF !== null) this.classes += ` SGR${toggleSGR.colorF}` // normal
    // backgrounds when blink is enabled
    if (toggleSGR.colorB !== null) this.classes += ` SGR${toggleSGR.colorB}`
    // presentation options this.classes
    if (toggleSGR.faint === true) this.classes += ` SGR2`
    if (toggleSGR.italic === true) this.classes += ` SGR3`
    if (toggleSGR.underline === true) this.classes += ` SGR4`
    if (toggleSGR.blinkSlow === true) this.classes += ` SGR5`
    if (toggleSGR.blinkFast === true) this.classes += ` SGR6`
    if (toggleSGR.inverse === true) this.classes += ` SGR7`
    if (toggleSGR.conceal === true) this.classes += ` SGR8`
    if (toggleSGR.crossedOut === true) this.classes += ` SGR9`
    if (toggleSGR.underlineX2 === true) this.classes += ` SGR21`
    if (toggleSGR.framed === true) this.classes += ` SGR51`
    if (toggleSGR.encircled === true) this.classes += ` SGR52`
    if (toggleSGR.overLined === true) this.classes += ` SGR53`
    // alternative fonts, values 11…19, 20 is reserved for a Gothic font
    // value 10 is the primary (default) font
    if (ecma48.font > 10 && ecma48.font <= 20) this.classes += ` SGR${ecma48.font}` // see `text_ecma_48.css` for fonts
    //console.log(`>input '${values}' output> this.classes: ${this.classes} bold: ${toggleSGR.bold}`) // uncomment to debug SGR
    if (verbose && vals === `SGR+38+2+0+103+108`) {
