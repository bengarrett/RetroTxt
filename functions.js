...
// @ ff  CSS font family class name
{
  ...
  if (ff.substring(0, 5) === "amiga" || ff.substring(0, 7) === "appleii" || ff.substring(0, 4) === "bios" || ff.substring(0, 3) === "c64" || ff.substring(0, 3) === "cga" || ff.substring(0, 7) === "cgathin" || ff.substring(0, 6) === "pc1512" || ff.substring(0, 5) === "tandy" || ff.substring(0, 6) === "vgalcd") {
    csslh = "normal";
    cssfs = `${fh}px`;
  } else {
    csslh = `${fh}px`;
...
function textShadow(s, elm, color)
...
// @color optional CSS colour class when we already know the new colour values
{
  try {
    ...
    console.error("textShadow(s, elm) the required parameter " + err);
    return;
  }
...
function txtConvert(s)
/*
 txtConvert takes text loaded as Windows-1252 or ISO 8859-1 but created for
...
// @s   String of text to convert
{
  ...
  var t = s,
    ctrlCodes = sessionStorage.getItem("cp437CtrlCodes"),
    fullConversion = true;
