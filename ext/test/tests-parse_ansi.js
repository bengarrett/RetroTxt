/*global Controls cursor domObject ecma48 italicElement reset resetCursor resetECMA resetSGR
...
QUnit.test(`CharacterAttributes class setBackground`, (t) => {
  ...
  t.equal(
    test._setBackground(),
    false,
    `SGR value \`abc\` is not a background`
  );
});
...
QUnit.test(`CharacterAttributes class setForeground`, (t) => {
  ...
  t.equal(
    test._setForeground(),
    false,
    `SGR value \`abc\` is not a foreground`
  );
});
