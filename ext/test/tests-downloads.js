    ...
    if (typeof chrome === 'undefined') {
      ...
      window.chrome = {
        ...
        downloads: {
          onCreated: {
            addListener: () => {
              /* Mock listener - intentionally empty */
            },
          },
          onChanged: {
            addListener: () => {
...
QUnit.test(
  'MockDownloads class - instantiation with monitor false',
  (assert) => {
...
QUnit.test('Downloads class - parseBlob with large file', (assert) => {
  ...
  assert.ok(
    typeof result === 'boolean',
    'Should return boolean for large files'
  );
  done();
});
...
QUnit.test('Downloads class - parseBlob with empty content', (assert) => {
  ...
  assert.ok(
    typeof result === 'boolean',
    'Should return boolean for empty content'
  );
  done();
});
...
QUnit.test('Downloads class - parseBlob with special characters', (assert) => {
  ...
  assert.ok(
    typeof result === 'boolean',
    'Should return boolean for special characters'
  );
  done();
});
...
QUnit.test('Downloads class - detect script tags', (assert) => {
  ...
  const downloads = new Downloads();
  const scriptBlob = new Blob(['<script>alert("test")</script>'], {
    type: 'text/plain',
  });
