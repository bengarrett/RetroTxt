...
QUnit.test('Downloads class - ASCII art file', async (assert) => {
  ...
  fetch('/test/example_files/downloads/ascii_art.txt')
    ...
    .then(async (content) => {
      ...
      assert.ok(
        content.includes('ABCDEFGHIJKLMNOPQRSTUVWXYZ'),
        'Should contain character sets'
      );

      const result = await downloads.parseBlob(blob, { tabid: 1 }, true);
      assert.equal(result, true, 'Should handle ASCII art file safely');
    ...
    fetch(`/test/example_files/downloads/${testFile.path}`)
      ...
      .then(() => {
        ...
        if (completed === testFiles.length) {
          ...
          assert.ok(
            matched >= testFiles.length * 0.8,
            `Should match most expectations: ${matched}/${testFiles.length}`
          );
      ...
      .catch((error) => {
        assert.ok(
          false,
          `Should analyze ${testFile.type} file: ${error.message}`
        );
        done();
      });
  });
  ...
  patternTests.forEach((patternTest) => {
    fetch(`/test/example_files/downloads/${patternTest.path}`)
      ...
      .then((content) => {
        ...
        const patternFound =
          content.includes(patternTest.pattern) ||
