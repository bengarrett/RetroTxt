...
QUnit.test(`Characters() class`, (assert) => {
  ...
  assert.equal(
    characters.compactIn(),
    `Mac OS Roman`,
    `MACINTOSH is supported`
  );
  characters = new Characters(Cs.UnicodeStandard);
...
QUnit.test(`FontFamily() class`, (assert) => {
  ...
  assert.equal(
    fonts.family,
    `ERROR`,
    `Invalid font name should return in caps`
  );
});
...
QUnit.test(`CreateLink() function`, (assert) => {
  ...
  let linkElement = `<link id="dummyId" href="chrome-extension://${
      extensionId
  ...
  linkElement = `<link href="chrome-extension://${
    extensionId
  }/test.css" type="text/css" rel="stylesheet"></link>`;
...
QUnit.test(`Titleize() function`, (assert) => {
  ...
  assert.equal(
    word,
    `Welcome To The IBM PC`,
    `Should be Welcome to the IBM PC`
  );
  word = Titleize(`welcome to the ibm pc`);
  assert.equal(
    word,
