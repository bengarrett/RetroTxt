...
class Tab {
  ...
  _compatibleURL() {
    ...
    try {
      ...
      pathname = parsedUrl.pathname; // Gets "/notes.txt", stripping ? and #
    } catch {
      console.warn(`Failed to parse URL for filename validation: ${this.url}`);
    }
    // URI object with a domain, a skip Boolean and scheme (https|file)
    const uri = {
      domain: this._hostname(),
      ignore: !config.validateFilename(pathname),
