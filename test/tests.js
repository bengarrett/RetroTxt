...
QUnit.test("ListMenuTitles", function(assert) {
  ...
  assert.equal(content[item], "MS-DOS CP-865", "Should be MS-DOS CP-865");
});
...
QUnit.test("ListCharacterSets", function(assert) {
  ...
  assert.equal(content.sets[0], "USASCII", "Should be `USASCII`");
});
...
QUnit.test("buildLinksToCSS", function(assert) {
  ...
  assert.throws(
    ...
    "When no `f` or `i` parameters are provided buildLinksToCSS() should throw an error"
  );
});
...
QUnit.test("FindTranscode", function(assert) {
  const content = new FindTranscode("CP437");
...
QUnit.test("BuildCharSet", function(assert) {
  ...
  assert.equal(content.guess, "USASCII", "Should be `USASCII`");
});

QUnit.test("changeOffTheme", function(assert) {
  assert.equal(changeOffTheme(), undefined, "Should return no value");
...
QUnit.test("FindDOM", function(assert) {
  const test = new FindDOM();
...
QUnit.test("ListCP865", function(assert) {
  const set = new ListCP865().set_a;
...
QUnit.test("SgrDefaults", function(assert) {
  const content = new SgrDefaults();
  assert.equal(content.colorF, 37, "Should be `37`");
  assert.equal(content.bold, false, "Should be `false`");
});

QUnit.test("ListCursorDefaults", function(assert) {
  const content = new ListCursorDefaults();
...
QUnit.test(`findControlCode()`, function(assert) {
  ...
  const sample = [48, 59, 51, 52, 109, 9556, 9552, 9552, 51, 72, , 32, 32, 32, 9553, 32, 9484],
    reply = `findControlCode should return`;
  let a = [];
  // CU* [1-9]
  gccTest([65]);
  ...
  // DSR
  gccTest([54, 110]);
  assert.equal(test, `DSR,2`, `'6n' should request the current cursor position`);
  // SCP
  gccTest([115]);
...
QUnit.test(`BuildEcma48()`, function(assert) {
  const reply = `sequence into HTML.`,
    ...
    outputText = `<div id=\"row-1\"><i class=\"SGR37 SGR40\">Hello world.</i></div>`;
  let sample = ``,
    uniqueResult = ``,
    test;
  // no controls
  sample = `Hello world.`;
  test = new BuildEcma48(sample).textAsHTML;
  ...
  sample = `←[2H${inputText}`;
  test = new BuildEcma48(sample).textAsHTML;
  uniqueResult = `<div id=\"row-1\"><i class=\"SGR37 SGR40\"> </i></div><div id=\"row-2\"><i id=\"column-1\" class=\"SGR0\"> </i><i class=\"SGR37 SGR40\">Hello world.</i></div>`;
  assert.equal(test, uniqueResult, `'${sample}' ${reply}`);
  ...
  sample = `←[2B${inputText}`;
  test = new BuildEcma48(sample).textAsHTML;
  uniqueResult = `<div id=\"row-1\"><i class=\"SGR37 SGR40\"> </i></div><div id=\"row-2\"><i class=\"SGR37 SGR40\"> </i></div><div id=\"row-3\"><i class=\"SGR37 SGR40\">Hello world.</i></div>`;
  assert.equal(test, uniqueResult, `'${sample}' ${reply}`);
  ...
  assert.equal(test.lineWrap, null, `'${sample}' lineWrap ${smReply}`);
  sample = `←[7h${inputText}`;
