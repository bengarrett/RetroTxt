...
/*global cursor domObject ecma48 italicElement reset
Build CharacterAttributes Cursor HTML Metadata Scan SGR Statistics */
"use strict"
...
QUnit.test(`Controls.parseFont()`, t => {
  const ansi = new Controls()
  t.equal(ansi.parseFont(), null)
  t.equal(ansi.parseFont(20), null)
  t.equal(ansi.parseFont(11), `bios`)
  t.equal(ansi.parseFont(`12`), `cga`)
  t.equal(ansi.parseFont(99), undefined)
  t.equal(ansi.parseFont(`twelve`), undefined)
})
...
QUnit.test(`Cursor.columnElement() async`, t => {
  ...
  const done0 = t.async()
  const done1 = t.async()
  const done10 = t.async()
  const doneNaN = t.async()
  setTimeout(() => {
...
QUnit.test(`CharacterAttributes.aixterm()`, t => {
  ...
  t.equal(test.aixterm(), 30, `aixterm() value \`60\` should return \`30\``)
  t.equal(test.toggles.bold, true, `aixterm() value \`60\` should toggle bold`)
  test.value = 100
  t.equal(test.aixterm(), 40, `aixterm() value \`40\` should return \`40\``)
  t.equal(
...
QUnit.test(`CharacterAttributes.rgb()`, t => {
  ...
  reset()
  test.value = 48
  test.values = [null, 48, 2, 256, 333, 9999]
  t.equal(test.rgb(), ``, `Is not a valid RGB value`)
})

QUnit.test(`CharacterAttributes.xtermBG()`, t => {
  const test = new CharacterAttributes()
  const goodValues = [480, 4811, 48255]
...
QUnit.test(`CharacterAttributes.xtermFG()`, t => {
  const test = new CharacterAttributes()
  const goodValues = [380, 3888, 38255]
  for (const v of goodValues) {
    test.value = v
    t.equal(
      test.xtermFG(),
...
QUnit.test(`CharacterAttributes.presentation()`, t => {
  ...
  const tests = [`bold`, `faint`, `italic`, `underline`]
  let val = 0
...
const testElmI = new Map()
  ...
  // SGR+[38|48][0-255], see functionsForArray() case `SGR`
...
QUnit.test(`HTML.element_i()`, t => {
  for (const [item, result] of testElmI.entries()) {
    const h = new HTML()
    h.item.value = `${item}`
    t.equal(h.element_i(), result, `'${item}' ${result}`)
  }
})
...
QUnit.test(`HTML.parseNamedSequence() HVP`, t => {
  ...
  t.equal(cursor.previousRow, 0, `Previous row should be 0`)
  t.equal(cursor.maxColumns, 80, `Max columns should be 80`)
  h.parseNamedSequence(0, `HVP+0+0`)
  t.equal(cursor.column, 1, `Cursor should remain at 1`)
  t.equal(cursor.row, 1, `Row should remain at 1`)
  t.equal(cursor.previousRow, 0, `Previous row should be 0`)
  t.equal(cursor.maxColumns, 80, `Max columns should be 80`)
  // test valid coordinates
  h.parseNamedSequence(0, `HVP+1+1`)
  t.equal(cursor.column, 1, `Cursor should remain at 1`)
  t.equal(cursor.row, 1, `Row should remain at 1`)
  t.equal(cursor.previousRow, 1, `Previous row should be 1`)
  t.equal(cursor.maxColumns, 80, `Max columns should be 80`)
  h.parseNamedSequence(0, `HVP+1+10`)
  t.equal(cursor.column, 10, `Cursor should be at 10`)
  h.parseNamedSequence(0, `HVP+10+10`)
  t.equal(cursor.column, 10, `Cursor should remain at 10`)
  t.equal(cursor.row, 10, `Row should remain at 10`)
  t.equal(cursor.previousRow, 1, `Previous row should be 1`)
  h.parseNamedSequence(0, `HVP+50+50`)
...
QUnit.test(`HTML.parseNamedSequence() SGR`, t => {
  ...
  h.parseNamedSequence(0, `SGR+1`)
  t.equal(
    domObject.html,
    ``,
    `DOM Object should remain empty while row is 1 or below`
  )
  h.parseNamedSequence(2, `SGR+1`)
  t.equal(
    domObject.html,
    `</i><i class="SGR137 SGR40">`,
    `DOM Object should contain an open <i> element`
  )
  h.parseNamedSequence(2, `SGR+2+5`)
  t.equal(
    domObject.html,
...
QUnit.test(`HTML.parseNamedSequence() ED+`, t => {
  const h = new HTML()
  h.parseNamedSequence(0, `ED+0`)
  t.deepEqual(cursor.eraseLines, [], `Should have no effect`)
  h.parseNamedSequence(0, `ED+1`)
  t.deepEqual(cursor.eraseLines, [], `Should not erase any rows`)
  h.parseNamedSequence(0, `ED+2`)
  t.deepEqual(cursor.eraseLines, [], `Should not erase any rows`)
  cursor.row = 2
  cursor.column = 2
  h.parseNamedSequence(0, `ED+1`)
...
QUnit.test(`HTML.parseNamedSequence() EL+`, t => {
  const h = new HTML()
  h.parseNamedSequence(0, `EL+0`)
  t.deepEqual(cursor.eraseLines, [], `Should have no effect`)
  cursor.row = 0
  h.parseNamedSequence(0, `EL+1`)
  t.deepEqual(cursor.eraseLines, [], `Should have no effect`)
  h.parseNamedSequence(0, `EL+2`)
  t.deepEqual(cursor.eraseLines, [], `Should have no effect`)
  cursor.row = 1
  h.parseNamedSequence(0, `EL+2`)
...
QUnit.test(`HTML.parseNamedSequence() SM+`, t => {
  const h = new HTML()
  h.parseNamedSequence(0, `SM+0`)
...
QUnit.test(`HTML.specialMarker()`, t => {
  for (const [item, result] of testSpecial.entries()) {
    const h = new HTML()
    domObject.html = ``
    t.equal(`${h.specialMarker(item)}`, result, `'${item}' ${result}`)
  }
})
...
QUnit.test(`Scan.functionsForArray()`, t => {
...
QUnit.test(`Scan.characterAttributes()`, t => {
  const input = text => {
  ...
  for (const [test, result] of testChrAttr.entries()) {
    ...
    const cp = input(test)
    t.equal(
      s.characterAttributes(0, cp, cp.length),
      result,
      `\`${test}\` should return "${result}"`
    )
  }
...
QUnit.test(`Scan.cursorCode()`, t => {
  for (const [test, result] of testCursorC.entries()) {
    const s = new Scan()
    t.equal(s.cursorCode(test), result, `[${test}] should return "${result}"`)
  }
})
...
QUnit.test(`Scan.cursorMove()`, t => {
  for (const [test, result] of testCursorM.entries()) {
    const s = new Scan()
    t.equal(
      s.cursorMove(test[0], test[1]),
      result,
      `[${test}] should return "${result}"`
    )
  }
...
QUnit.test(`Scan.cursorHuman()`, t => {
  for (const [test, result] of testCursorH.entries()) {
    const s = new Scan()
    t.equal(s.cursorHuman(test), result, `[${test}] should return "${result}"`)
  }
})
...
QUnit.test(`Scan.cursorName()`, t => {
  for (const [test, result] of testCursorN.entries()) {
    const s = new Scan()
    t.equal(s.cursorName(test), result, `[${test}] should return "${result}"`)
  }
})
...
QUnit.test(`Scan.digit()`, t => {
  for (const [test, result] of testDigit.entries()) {
    const s = new Scan()
    t.equal(s.digit(test), result, `[${test}] should return "${result}"`)
  }
})
...
QUnit.test(`Scan.cursorHVP()`, t => {
  for (const [test, result] of testCursorHVP.entries()) {
    const s = new Scan()
    t.equal(s.cursorHVP(test), result, `[${test}] should return "${result}"`)
  }
})
...
QUnit.test(`Scan.pabloDrawRGB()`, t => {
  for (const [test, result] of testPabloDraw.entries()) {
    const s = new Scan()
    t.equal(s.pabloDrawRGB(test), result, `[${test}] should return "${result}"`)
  }
})
...
QUnit.test(`Scan.setMode()`, t => {
  for (const [test, result] of testSM.entries()) {
    const s = new Scan()
    t.equal(
      s.setMode(test[0], test[1], test[2]),
      result,
      `[${test}] should return "${result}"`
    )
  }
...
QUnit.test(`Build.decimals()`, t => {
  for (const [test, result] of testDec.entries()) {
    const b = new Build(test)
    t.deepEqual(b.decimals(), result, `"${test}" should return [${result}]`)
  }
})
