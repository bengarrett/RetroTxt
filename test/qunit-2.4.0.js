...
// CSpell:ignore unskipped Philippe Rathé prathe Flesler notrycatch noglobals
...
(function(global$1) {
  ...
  Test.prototype = {
    ...
    pushResult: function pushResult(resultInfo) {
      ...
      // De-structure of resultInfo = { result, actual, expected, message, negative }
      var source,
        details = {
  ...
  function begin() {
    ...
    if (!config.started) {
      // Record the time of the test run beginning
      config.started = now()
  ...
  (function() {
    ...
    function addEvents(elms, type, fn) {
      var i = elms.length
      while (i--) {
        addEvent(elms[i], type, fn)
      }
    }
  ...
  QUnit.diff = (function() {
     ...
     * any common prefix or suffix off the texts before diff-ing.
      ...
      while (pointer < diffs.length) {
        if (diffs[pointer][0] === DIFF_EQUAL) {
          ...
          if (
            ...
            // Throw away the previous equality (it needs to be re-evaluated).
            equalitiesLength--
            pointer =
              equalitiesLength > 0 ? equalities[equalitiesLength - 1] : -1
     ...
     *     The zero element of the array of unique strings is intentionally blank.
     * @private
     */
